...
! Initial condition
  dx = TWOPI/DBLE(nn)
  DO kk = 1, NDIM
     U(kk) = 1.0d-3*DSIN(dx*DBLE(kk)*1.0d0)
  END DO
...
! normally unused jacobian arguments : ijac, dbc (see manual)
  ...
  REAL(DP), INTENT(IN) :: PAR(*), U0(NDIM), U1(NDIM)
  REAL(DP), INTENT(OUT) :: FB(NBC)
  REAL(DP), INTENT(INOUT) :: DBC(NBC,*)
  ...
  DO kk=1, NDIM
     tmp = tmp + U0(kk)*DCOS(dx*DBLE(kk))
  END DO
  tmp = tmp - SUM(U0(1:NDIM))
  FB(1) = tmp*dx

...
! obtain h_fft
  ...
  h_fft(2) = DCMPLX(0.0d0, U(1))
  IF(MOD(nn,2).EQ.0)THEN
     DO kk = 3, nn/2
        h_fft(kk) = DCMPLX(U(2*kk-4), U(2*kk-3))
     END DO
     h_fft(nend) = DCMPLX(U(NDIM), 0.0d0)
  ELSE
     DO kk = 3, (nn+1)/2
        h_fft(kk) = DCMPLX(U(2*kk-4), U(2*kk-3))
     END DO
...
! Compute h2_fft: Fourier(h^2)
  ...
  IF(DABS(U(1)).LT.1.0d-5)THEN
     cc = 0.0d0
  ELSE
     cc = 0.5d0*IMAG(h2_fft(2))/U(1)
  END IF
  ...
  IF(MOD(nn,2).EQ.0)THEN
     DO kk = 3, nn/2
        F(2*kk-4) = DREAL(F_fft(kk))
        F(2*kk-3) = DIMAG(F_fft(kk))
     END DO
     F(NDIM) = DREAL(F_fft(nn/2+1))
  ELSE
     DO kk = 3, (nn+1)/2
        F(2*kk-4) = DREAL(F_fft(kk))
        F(2*kk-3) = DIMAG(F_fft(kk))
...
!  nu = 1.0d0
  ...
  PAR(1) = nu       ! coefficient of the fourth derivative term
...
!     ---------- ----
  ...
  END DO
  PAR(45) = DSQRT(sumall)/DBLE(NDIM+2)

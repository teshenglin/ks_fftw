...
! Compute d1h (h_x) ~ d4h (h_xxxx):
  ...
  dx = TWOPI/DBLE(nn)
  DO kk=1, nn
     tmp = tmp + hh(kk)*DSIN(1.0d0*dx*DBLE(kk-1))
     tmp2 = tmp2 + (hh(kk)**2)*DSIN(1.0d0*dx*DBLE(kk-1))
  END DO
  IF(DABS(tmp).LT.1.0d-4)THEN
     cc = 0.0d0
  ELSE
     cc = 0.5d0*tmp2/tmp
  END IF
  ...
  DO kk=2, nn
     F(kk-1) = cc*d1h(kk) - hh(kk)*d1h(kk) - d2h(kk) - nu*d4h(kk)
  END DO
...
! Initial condition
  dx = TWOPI/DBLE(nn)
  DO kk = 1, NDIM
     U(kk) = 1.0d-3*DSIN(dx*DBLE(kk)*2.0d0)
  END DO
...
!---------------------------------------------------------------------- 
  ...
  tmp = 0.0d0
  tmp2 = 0.0d0
  dx = TWOPI/DBLE(NDIM+1)
  DO kk=1, NDIM
     tmp = tmp + getp('INT', kk, u)*DSIN(1.0d0*dx*DBLE(kk))
     tmp2 = tmp2 + (getp('INT', kk, u)**2)*DSIN(1.0d0*dx*DBLE(kk))
  END DO
  IF(DABS(tmp).LT.1.0d-4)THEN
     PAR(6) = 0.0d0
  ELSE
     PAR(6) = 0.5d0*tmp2/tmp
  END IF
